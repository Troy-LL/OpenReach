...
Production is the Worker named `openreach` on Troy's personal Cloudflare account, custom domain `https://openreach.niched.tech` (zone `niched.tech`, same pattern as `pupsync` and `may-pasok-ba`).

Build the UI, then deploy:
...
That runs `vite build` and `wrangler deploy`. Wrangler attaches the custom domain from `wrangler.jsonc`. The `*.workers.dev` URL is a fallback; use `https://openreach.niched.tech` in production.
